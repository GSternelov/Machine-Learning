...
The code that has been used to implement the functions can be seen in the appendix *R-code* at the end of the report.  
...
```
  ...
  The probability bands are more narrow around the observed value.
...
Compared to the plots in *b)* and *c)*, the curve for the posterior mean of *f* is less straight/ more curvaceous than before. The probability bands has also changed and are thanks to the rise from two to five observed values more narrow.
...
```
  ...
  Compared to the plot in *d)*, the probability bands obtained with $\iota$ set to 1 looks much smoother and lies much closer to the curve for the posterior mean of *f*.  
...
par(mfrow=c(1,1))
```
It can be seen that better fits are given for lower values of $\sigma_n$. A lower noise standard deviation gives better results, both a better fit and narrower probability bands.  

A bad fit for the highest value, a little bit better when equal to 5 and a quite good fit when $\sigma_n$ is equal to 2.  
What exactly is it that $\sigma_n$ affects? Is it flexibility/smoothness, larger variance affects certainty? 
...
par(mfrow=c(1,1))
```
For a $\iota$ value of 0.05 the obtained curve is too flexible, it follows the observd values too well. For a little higher value, 0.30, the curve is more smooth and seem to be a quite good fit. When a even higher value is tested, 0.75, the curve instead becomes to unflexible.  


# Appendix
## R-code
```{r code=readLines(knitr::purl('C:/Users/Gustav/Documents/Machine-Learning/Lab 6/Lab6.rmd', documentation = 0)), eval = FALSE}
