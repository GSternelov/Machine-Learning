...
## Assignment 1
### 1.1
The function *ridgereg_nfoldCV* which performs ridge regression by using n-fold cross validation is implemented at the first step of assignment 1. The R-code used to create the function can be seen below. 
```{r 1.1,eval=TRUE}
...
## Assignment 2
In the second assignment the data material called *tecator* is used. This file contains information from a study where...  
...
For models with polynomials of higher power the MSE for test data in general increases, and for training data it decreases. The best model for training data in terms of MSE is the, probably overfitted, model with polynomial terms up to a power of 6. This can be compared to the the test data which has the lowest MSE for a simple linear model. In terms of the bias/variance trade-off the plot an interpretation of the plot can be that...
...
```
Compare the plots, conclusions
The difference between a Ridge regression model and a LASSO model... 
...
How the CV score depend on the log of $\lambda$  
