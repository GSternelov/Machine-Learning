...
The conclusion is that some knowledge (but how much?) about the expected response is gained by the model. The interval in general lies fairly close to the observed valeus, this should be rather good? A lot of the observed values are at the edge of the confidence interval, but none is outside it. 
...
```
What can ve said about the models appropriateness? 
Almost all of the original values, 46 out of 48, lies inside the confidence bands. That's pretty okay? By the look it may be a reasonable model? Or are the prediction bands to wide(or small?)? It is hard to give an proper evaluation of the appropriateness of the model...The parametric bootstrap follows a normal distribution, that should not be reasonable?  

Although with in mind that it is 95% predicition bands it can be concluded that more than 5% of the values lies outside the estimated bands. A conclusion that not speaks in favor of the appropriateness of the model. 

### 1.5
What kind of bootstrap is more appropriate?
What is the goal with the confidence bands? Should they capture the pattern of data (if so, be a bit bumby in this case, or is that overfittness? or something else that is bad?) Or should they capture the range the values in most cases lies within? (similar to what the prediction bands gave)
...
Has it to do with transformation(rotation) ? The first principal component is changed a lot, the second barely changed at all. This has got to do with the values in W, but how?
