...
ggplot(lda_class, aes(y=RW, x=CL)) + 
  geom_point(aes(color=classif,shape=sex), size=4)

```

**plot also the decision boundary. Comment on the quality of fit.**  
...
ggplot(pred_logi, aes(y=aussieCrab.RW, x=aussieCrab.CL)) + 
  geom_point(aes(color=pred_logi[,4],shape=aussieCrab.sex), size=4)

table(pred_logi[,4], pred_logi$aussieCrab.sex)
```

**plot the classified data and present the equation of the decision boundary. Compare this result with the LDA result.**
