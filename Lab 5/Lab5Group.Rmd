...
author: "Martina, Araya, Andrea, Gustav Sternelöv"
...
## 1.2 - 1.3
\begin{equation}
 K_{\lambda}(x,x_0)=\frac{3}{4}\left(1- \left( \frac{|x-x_0|}{\lambda}\right)^2 \right) I\left( \frac{|x-x_0|}{\lambda} <1 \right)  
 \end{equation}\

\begin{equation}
\hat{f}(x_0)=\frac{\sum_{i=1}^N{K_{\lambda}(x_i,x_0)y_i}}{\sum_{i=1}^N{K_{\lambda}(x_i,x_0)}}  
 \end{equation}\
...
The predicted curve for the SVM model seem to be a bad fit, especially when it is compared to predicted curve for the model from 1.3c)(the model with $\lambda$ equal to 10). 
The MSE for the SVM model is `r MSE_SVM` and for the best model in 1.3 the MSE was `r MSEC`. In the light of both better looking prediction curves and lower MSE the best model from 1.3 undoubtedly seem to be a better fit than the SVM model.
...
In step d the value of sigma has the concrete effect that the amount of support vectors increases. That is because a relatively large sigma, like in this case, gives a larger margin width and all observations that lies inside the margin lines becmoes support vectors.(?)
...
\[  \mbox{Misclassification rate} = \frac{5+5}{5+5+313+98+151} = 0.0175  \]\
